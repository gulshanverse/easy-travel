/**
 * Decision telemetry — metrics, structured logging, tracing, health.
 * Provider-independent. Adapters wired in composition root.
 */
...
export interface DecisionTelemetry {
  ...
  startSpan(name: string, attrs?: Record<string, unknown>): DecisionSpan;
}

export function createNoopTelemetry(): DecisionTelemetry {
  const span: DecisionSpan = { end() {}, setAttribute() {} };
  return { emit() {}, startSpan() { return span; } };
}

export function createConsoleTelemetry(prefix = "decision"): DecisionTelemetry {
  return {
    emit(level, message, attrs) {
      const line = `[${prefix}] ${level.toUpperCase()} ${message}`;
      // eslint-disable-next-line no-console
      (console[level === "warn" ? "warn" : level === "error" ? "error" : "log"] as (...a: unknown[]) => void)(
        line, attrs ? { attrs } : "",
      );
    },
...
export function createInMemoryMetrics(): DecisionMetrics {
  ...
  const key = (n: string, t?: Record<string, string>) =>
    t && Object.keys(t).length
      ? `${n}#${Object.entries(t).map(([k, v]) => `${k}=${v}`).sort().join(",")}`
      : n;
  return {
