...
export class MemoryLifecycleManager {
  ...
  async archive(memoryId: string, ownerId: string, actor: LifecycleActor): Promise<MemoryEnvelope> {
    ...
    const updated = await this.store.patch(memoryId, {
  ...
  async softDelete(
    ...
    const recoverableUntil = new Date(
      Date.now() + policy.softDeleteGraceSeconds * 1000,
    ).toISOString();
    const updated = await this.store.patch(memoryId, {
      status: "deleted",
      version: env.version + 1,
  ...
  async restore(memoryId: string, ownerId: string): Promise<MemoryEnvelope> {
    ...
    if (env.status !== "deleted")
      throw new MemoryConflictError("only soft-deleted memories can be restored");
    const updated = await this.store.patch(memoryId, {
      status: "active",
      version: env.version + 1,
    });
  ...
  async supersede(oldId: string, newId: string, ownerId: string): Promise<void> {
    ...
    this.publisher.publish(
      ...
      {
        memoryId: oldId,
        changedFields: ["status", "supersededBy"],
        priorVersion: old.version,
