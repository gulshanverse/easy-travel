...
export function registerPayloadValidator(class_: MemoryClass, kind: string, fn: PayloadValidator): void {
...
export class MemoryValidators {
  static validateDraft<T>(draft: MemoryDraft<T>): void {
    ...
    assert(typeof draft.kind === "string" && KEBAB_RE.test(draft.kind), `kind must be kebab-case, got: ${String(draft.kind)}`);
    ...
    assert(typeof draft.source.kind === "string" && typeof draft.source.actorId === "string", "source.kind and source.actorId are required");
    if (draft.confidence !== undefined) assert(isBoundedNumber(draft.confidence), "confidence must be in [0,1]");
    if (draft.importance !== undefined) assert(isBoundedNumber(draft.importance), "importance must be in [0,1]");
    if (draft.evidence) {
    ...
    if (draft.tags) {
      ...
      for (const t of draft.tags) assert(typeof t === "string" && KEBAB_RE.test(t), `tag must be kebab-case: ${String(t)}`);
    }
    if (draft.relationships) {
      ...
      for (const r of draft.relationships) {
        assert(typeof r?.type === "string" && typeof r.targetId === "string", "relationships[].type and .targetId required");
