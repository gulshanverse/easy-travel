...
const obj = (v: unknown, what: string): Raw => {
  if (!v || typeof v !== "object")
    throw new MultiModalNormalizationError(`expected object payload for ${what}`);
  return v as Raw;
};
...
const b = (r: Raw, k: string, d = false): boolean =>
...
function airport(r: Raw): NormalizedAirport {
  return freezeModel({
    ...
    lon: n(r, "lon"),
    timezone: s(r, "tz"),
    terminals: n(r, "terminals", 1),
  });
}
function flight(r: Raw): NormalizedFlight {
  const dep = n(r, "dep_min");
  const arrM = n(r, "arr_min");
  const duration = arrM >= dep ? arrM - dep : 1440 - dep + arrM;
  return freezeModel({
    flightNumber: s(r, "flight_no"),
    carrier: s(r, "carrier_code"),
    fromCode: s(r, "origin"),
    toCode: s(r, "destination"),
    departureMinutes: dep,
    arrivalMinutes: arrM,
    durationMinutes: duration,
    stops: n(r, "stops"),
    aircraft: s(r, "aircraft_type"),
    cabins: list(r, "cabin_classes"),
    cost: makeTravelCost({
      amount: n(r, "base_fare"),
...
function hotel(r: Raw): NormalizedHotel {
  return freezeModel({
    ...
    nightlyCost: makeTravelCost({
      amount: n(r, "nightly_rate"),
      currency: s(r, "currency", "INR"),
      label: "nightly",
    }),
  });
...
function room(r: Raw): NormalizedRoom {
  return freezeModel({
    id: s(r, "room_id"),
    hotelId: s(r, "hotel_id"),
    name: s(r, "name"),
    capacity: n(r, "capacity", 1),
    bedType: s(r, "bed_type"),
    cost: makeTravelCost({
      amount: n(r, "price"),
      currency: s(r, "currency", "INR"),
      label: "nightly",
...
function weather(r: Raw): NormalizedWeather {
  return freezeModel({
    ...
    temperatureC: n(r, "temp_c"),
    feelsLikeC: n(r, "feels_c"),
    humidity: n(r, "humidity"),
    windKph: n(r, "wind_kph"),
    windBearing: n(r, "wind_deg"),
    visibilityM: n(r, "visibility_m"),
    rainProbability: n(r, "rain_prob"),
    airQualityIndex: n(r, "aqi"),
...
function forecast(r: Raw): NormalizedForecast {
  return freezeModel({
    ...
    points: arr(r.points).map((p) => ({
      ...
      temperatureC: n(p, "temp_c"),
      rainProbability: n(p, "rain_prob"),
      windKph: n(p, "wind_kph"),
    })),
  });
}
function alerts(r: Raw) {
  return freezeModel(
    arr(r.alerts).map((a) => ({
      id: s(a, "alert_id"),
      place: s(a, "place"),
      kind: s(a, "kind", "advisory"),
      severity: s(a, "severity", "info"),
      message: s(a, "message"),
      issuedAt: n(a, "issued_at"),
    })),
  );
}
...
function matrix(r: Raw): NormalizedDistanceMatrix {
  return freezeModel({
    cells: arr(r.cells).map((cl) => ({
      origin: s(cl, "origin"),
      destination: s(cl, "destination"),
      distanceKm: n(cl, "distance_km"),
      minutes: n(cl, "minutes"),
    })),
  });
}
function timezone(r: Raw) {
  return freezeModel({
    id: s(r, "tz_id"),
    place: s(r, "place"),
    offsetMinutes: n(r, "offset_min"),
    abbreviation: s(r, "abbr"),
    dst: b(r, "dst"),
  });
}
function localTime(r: Raw) {
  return freezeModel({
    place: s(r, "place"),
    timezone: timezone(obj(r.tz, "localTime.tz")),
    instant: n(r, "instant"),
    localIso: s(r, "local_iso"),
  });
}
...
export function normalizeTravelPayload(capability: MultiModalCapabilityId, raw: unknown): unknown {
  ...
  switch (capability) {
    case "search_airports":
      return freezeModel(arr(r.airports).map(airport));
    case "search_flights":
      return freezeModel(arr(r.flights).map(flight));
    case "flight_status":
      return freezeModel({
        flightNumber: s(r, "flight_no"),
        state: s(r, "state", "scheduled"),
        delayMinutes: n(r, "delay_min"),
        gate: s(r, "gate"),
        terminal: s(r, "terminal"),
    ...
    case "flight_schedule":
      return freezeModel({
        ...
        fromCode: s(r, "origin"),
        toCode: s(r, "destination"),
        legs: arr(r.legs).map((l) => ({
          day: s(l, "day"),
          departureMinutes: n(l, "dep_min"),
          arrivalMinutes: n(l, "arr_min"),
        })),
      });
    ...
    case "fare_lookup":
      return freezeModel(
        arr(r.fares).map((f) =>
          makeTravelCost({
            amount: n(f, "amount"),
            currency: s(f, "currency", "INR"),
            label: s(f, "cabin"),
            refundable: b(f, "refundable"),
            breakdown: { base: n(f, "base"), taxes: n(f, "taxes") },
          }),
        ),
      );
    ...
    case "search_hotels":
      return freezeModel(arr(r.hotels).map(hotel));
    case "hotel_metadata":
      return hotel(obj(r.hotel, "hotel"));
    case "search_rooms":
      return freezeModel(arr(r.rooms).map(room));
    case "hotel_availability":
      return freezeModel({
        hotelId: s(r, "hotel_id"),
        available: b(r, "available"),
        roomsLeft: n(r, "rooms_left"),
    ...
    case "hotel_amenities":
      return freezeModel([...list(r, "amenities")]);
    case "geocode":
    case "reverse_geocode":
    case "coordinates":
      return location(obj(r.location, capability));
    case "search_places":
      return freezeModel(arr(r.places).map(place));
    case "distance_matrix":
      return matrix(r);
    case "route":
      return route(r);
    case "travel_time":
      return makeTravelDuration(n(r, "minutes"), s(r, "mode"), n(r, "confidence", 0.8));
    case "region_lookup":
      return freezeModel({
        id: s(r, "region_id"),
        name: s(r, "name"),
        country: s(r, "country"),
        level: s(r, "level", "city"),
    ...
    case "local_transport":
      return freezeModel(arr(r.options).map(transit));
    case "transit_modes":
      return freezeModel([...list(r, "modes")]);
    case "exchange_rate":
    ...
    case "travel_budget_currency":
      return freezeModel({
        homeCurrency: s(r, "home"),
        destinationCurrency: s(r, "dest"),
        homeAmount: n(r, "home_amount"),
        destinationAmount: n(r, "dest_amount"),
        rate: n(r, "rate"),
      });
