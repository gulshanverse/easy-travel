...
): TravelProviderRawResult => Object.freeze({ ok: true, data, pagination });
...
const str = (i: TravelRequestInput, k: string, d = ""): string =>
  typeof i[k] === "string" ? (i[k] as string) : d;
const num = (i: TravelRequestInput, k: string, d = 0): number =>
  typeof i[k] === "number" ? (i[k] as number) : d;

function profile(
  id: string,
  name: string,
  mode: TravelMode,
  extra: Partial<TravelProviderProfile> = {},
): TravelProviderProfile {
...
function baseAdapter(
  ...
  return Object.freeze({
    profile: p,
    supports(capability: MultiModalCapabilityId) {
...
// ------------------------------------------------------------------ Flight
export function createMockFlightProvider(
  options: { id?: string; failCapability?: MultiModalCapabilityId } = {},
): FlightProvider {
  ...
  const run = (
    ...
    if (options.failCapability === capability)
      return fail("mock_forced_failure", `forced failure for ${capability}`, true);
    switch (capability) {
      case "search_airports": {
        const q = str(input, "query").toLowerCase();
        const limit = num(input, "limit", 20);
        const matches = ds.airports
          .filter(
            (a) =>
      ...
      case "search_flights": {
        ...
        if (matches.length === 0)
          matches = ds.flights.filter((f) => f.from === from).slice(0, limit);
        if (matches.length === 0) matches = ds.flights.slice(0, limit);
        return ok(
          {
            flights: matches.slice(0, limit).map((f) => ({
              flight_no: f.number,
      ...
      case "flight_status": {
        ...
        const state =
          u > 0.95 ? "cancelled" : delay > 0 ? "delayed" : u > 0.5 ? "en_route" : "scheduled";
        return ok({
          flight_no: no,
          state,
          delay_min: delay,
          gate: `G${1 + Math.floor(u * 40)}`,
          terminal: `T${1 + Math.floor(u * 3)}`,
          updated_at: 0,
        });
      }
      case "flight_schedule": {
        ...
        return ok({
          ...
          legs: days.map((d, i) => ({
            day: d,
            dep_min: (f.dep + i * 3) % 1440,
            arr_min: (f.arr + i * 3) % 1440,
          })),
        });
      }
      ...
      case "flight_delay_information": {
        ...
        return ok({
          ...
          cause:
            delay === 0
              ? "none"
              : ["weather", "atc", "rotation", "technical"][Math.floor(u * 4) % 4],
  ...
  const run = (
    ...
    switch (capability) {
      case "search_hotels": {
        ...
        const limit = num(input, "limit", 20);
        let matches = city
          ? ds.hotels.filter((h) => h.city.toLowerCase().includes(city))
          : ds.hotels;
      ...
      case "search_rooms": {
        ...
        return ok({
          rooms: h.rooms
            .filter((r) => r.capacity >= guests || guests <= 1)
            .map((r) => ({
              room_id: r.id,
              hotel_id: h.id,
              name: r.name,
...
function rawLocation(name: string) {
  ...
  return {
    ...
    lon: c.lon,
    country: c.country,
    region: `${c.name} Metropolitan Area`,
    tz: c.timezone,
  };
}
  ...
  const run = (
    ...
    switch (capability) {
      ...
      case "search_places": {
        ...
        const limit = num(input, "limit", 20);
        const matches = ds.places
          .filter(
            (pl) =>
              (pl.name.toLowerCase().includes(q) ||
                pl.city.toLowerCase().includes(q) ||
                pl.category.includes(q)) &&
      ...
      case "route": {
        ...
        const a = cityOf(str(input, "from"));
        const b = cityOf(str(input, "to"));
        const km = haversineKm(a.lat, a.lon, b.lat, b.lon);
        ...
        return ok({
          ...
          from: rawLocation(a.name),
          to: rawLocation(b.name),
          mode,
          distance_km: km,
          minutes,
      ...
      case "travel_time": {
        ...
        const km = haversineKm(a.lat, a.lon, b.lat, b.lon);
        return ok({
          minutes: Math.max(1, Math.round((km / speedKph(mode)) * 60)),
          mode,
          confidence: 0.86,
        });
      }
...
export function createMockWeatherProvider(options: { id?: string } = {}): WeatherProvider {
  ...
  const measure = (place: string, metric: string) => {
    ...
    switch (metric) {
      case "temperature":
        return { metric, value: Math.round((2 + u * 38) * 10) / 10, unit: "C" };
      case "wind":
        return { metric, value: Math.round(u * 60), unit: "kph" };
      case "visibility":
        return { metric, value: 500 + Math.round(u * 9500), unit: "m" };
      case "rain_probability":
        return { metric, value: Math.round(u * 100), unit: "%" };
      case "air_quality":
        return { metric, value: 10 + Math.round(u * 240), unit: "aqi" };
      default:
        return { metric, value: Math.round(u * 100), unit: "" };
    }
  };
  ...
  const alerts = (place: string, only: "storm" | "all") => {
    ...
    if (u > 0.55) {
      list.push({
        alert_id: `alr_${place}_storm`,
        place,
        kind: "storm",
        severity: u > 0.85 ? "severe" : "moderate",
        message: `Thunderstorm activity expected near ${place}.`,
        issued_at: 0,
      });
    }
    if (only === "all" && u > 0.3) {
      list.push({
        alert_id: `alr_${place}_advisory`,
        place,
        kind: "advisory",
        severity: "minor",
        message: `Allow extra transfer time in ${place}.`,
        issued_at: 0,
      });
    }
    return list;
  ...
  const run = (
    ...
    switch (capability) {
      ...
      case "forecast_daily": {
        ...
        const count = hourly
          ? Math.max(1, num(input, "hours", 12))
          : Math.max(1, num(input, "days", 7));
      ...
      case "travel_alerts":
        return ok({ alerts: alerts(place, "all") });
      case "storm_alerts":
        return ok({ alerts: alerts(place, "storm") });
      case "rain_probability":
        return ok(measure(place, "rain_probability"));
      case "visibility":
        return ok(measure(place, "visibility"));
      case "temperature":
        return ok(measure(place, "temperature"));
      case "wind":
        return ok(measure(place, "wind"));
      case "air_quality":
        return ok(measure(place, "air_quality"));
      default:
        return fail("mock_unsupported", `unsupported: ${capability}`);
    }
  };
    ...
    switch (capability) {
      case "transit_modes": {
        ...
        const available = ALL.filter(
          (m, i) => m === "walking" || m === "taxi" || (i + Math.floor(u * 10)) % 3 !== 0,
        );
...
export function createMockCurrencyProvider(options: { id?: string } = {}): CurrencyProvider {
  ...
  const rate = (from: string, to: string, daysAgo = 0): number => {
    const f = ds.rates[from.toUpperCase()];
    const t = ds.rates[to.toUpperCase()];
    if (!f || !t) return 0;
  ...
  const run = (
    ...
    switch (capability) {
      ...
      case "travel_budget_currency": {
        ...
        if (!r) return fail("mock_unknown_currency", `unknown currency pair ${from}/${to}`);
        return ok({
          home: from.toUpperCase(),
          dest: to.toUpperCase(),
          home_amount: amount,
          dest_amount: Math.round(amount * r * 100) / 100,
          rate: r,
        });
      }
      default:
