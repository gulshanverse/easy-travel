...
function monitorPnr(): WorkflowDefinition {
  return makeWorkflowDefinition({
    ...
    steps: [
      {
        id: "fetch",
        name: "Fetch PNR status",
        kind: "connector",
        connectorId: RAIL,
        capabilityId: "railway.pnr.status",
        dependsOn: [],
      },
      {
        id: "evaluate",
        name: "Evaluate PNR change",
        kind: "capability",
        capabilityId: "workflow.evaluate.pnr",
        dependsOn: ["fetch"],
      },
      {
        id: "notify",
        name: "Record PNR change",
        kind: "capability",
...
function monitorSeatAvailability(): WorkflowDefinition {
  return makeWorkflowDefinition({
    id: "builtin.monitor-seat-availability",
    name: "Monitor Seat Availability",
    version: "1.0.0",
    triggers: [{ kind: "schedule", intervalMs: 30 * 60_000 }],
    steps: [
      {
        id: "availability",
        name: "Fetch seat availability",
        kind: "connector",
        connectorId: RAIL,
        capabilityId: "railway.seat.availability",
        dependsOn: [],
      },
      {
        id: "compare",
        name: "Compare with target class",
        kind: "capability",
        capabilityId: "workflow.compare.availability",
        dependsOn: ["availability"],
      },
      {
        id: "record",
        name: "Record availability delta",
        kind: "capability",
        capabilityId: "workflow.record.change",
        dependsOn: ["compare"],
      },
...
function trackLiveTrain(): WorkflowDefinition {
  return makeWorkflowDefinition({
    id: "builtin.track-live-train",
    name: "Track Live Train",
    version: "1.0.0",
    ...
    steps: [
      {
        ...
        kind: "connector",
        connectorId: RAIL,
        capabilityId: "railway.live.status",
        dependsOn: [],
      },
      {
        id: "project",
        name: "Project arrival",
        kind: "capability",
        capabilityId: "workflow.project.arrival",
        dependsOn: ["live"],
      },
    ],
  });
  ...
  return makeWorkflowDefinition({
    ...
    steps: [
      ...
      },
      {
        ...
        kind: "agent",
        capabilityId: "travel-orchestrator",
        dependsOn: ["wait"],
      },
      {
        id: "record",
...
function trainDelayWatch(): WorkflowDefinition {
  return makeWorkflowDefinition({
    id: "builtin.train-delay-watch",
    name: "Train Delay Watch",
    version: "1.0.0",
    ...
    steps: [
      {
        ...
        kind: "signal",
        signalName: "train.delay",
        dependsOn: [],
      },
      {
        id: "delay",
        name: "Fetch delay detail",
        kind: "connector",
        connectorId: RAIL,
        capabilityId: "railway.delay",
        dependsOn: ["await"],
      },
      {
        id: "assess",
        name: "Assess journey impact",
        kind: "capability",
...
function platformChangeWatch(): WorkflowDefinition {
  return makeWorkflowDefinition({
    id: "builtin.platform-change-watch",
    name: "Platform Change Watch",
    version: "1.0.0",
    triggers: [{ kind: "signal", name: "platform.change" }],
    steps: [
      {
        id: "await",
        name: "Await platform signal",
        kind: "signal",
        signalName: "platform.change",
        dependsOn: [],
      },
      {
        id: "platform",
        name: "Fetch platform info",
        kind: "connector",
        connectorId: RAIL,
        capabilityId: "railway.platform",
        dependsOn: ["await"],
      },
      {
        id: "record",
        name: "Record platform change",
        kind: "capability",
        capabilityId: "workflow.record.change",
        dependsOn: ["platform"],
...
export const BUILTIN_WORKFLOW_IDS: readonly string[] = Object.freeze([
  "builtin.monitor-pnr",
  "builtin.monitor-seat-availability",
  "builtin.track-live-train",
  "builtin.journey-reminder",
  "builtin.train-delay-watch",
  "builtin.platform-change-watch",
]);
