...
export const WORKFLOW_TRANSITIONS: Readonly<Record<WorkflowLifecycleState, readonly WorkflowLifecycleState[]>> =
  Object.freeze({
...
export function isTerminal(state: WorkflowLifecycleState): boolean {
  return state === "completed" || state === "cancelled" || state === "failed" || state === "archived";
}
