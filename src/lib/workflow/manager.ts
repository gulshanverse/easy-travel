...
import type {
  WorkflowDefinition, WorkflowExecution, WorkflowInstance, WorkflowSnapshot,
  WorkflowStatistics, WorkflowVariables,
} from "./types";
...
export class WorkflowManager {
  ...
  constructor(private readonly deps: WorkflowManagerDeps) {
    ...
    this.engine = new WorkflowStateMachineEngine({
      events: deps.events, metrics: deps.metrics, telemetry: deps.telemetry,
      ctor: deps.ctor, agent: deps.agent, integration: deps.integration,
    ...
    this.scheduler.onDue(async entry => {
      ...
      if (instanceId && this.instances.has(instanceId)) {
        ...
        deps.events.emit({ name: "TimerFired", instanceId, at: deps.clock.now(), data: { entryId: entry.id } });
        await this.fireTimer(instanceId);
      } else if (definitionId && this.registry.has(definitionId)) {
        deps.events.emit({ name: "ScheduledEvent", definitionId, at: deps.clock.now(), data: { entryId: entry.id } });
        const inst = this.create(definitionId, (entry.payload.variables as WorkflowVariables) ?? {});
  ...
  register(def: WorkflowDefinition): WorkflowDefinition {
    ...
    this.deps.events.emit({ name: "WorkflowRegistered", definitionId: d.id, at: this.deps.clock.now() });
    return d;
  }

  // ---------- instances ----------
  create(definitionId: string, variables: WorkflowVariables = {}, priority?: number): WorkflowInstance {
    ...
    const active = this.list().filter(i => i.definitionId === def.id && !isTerminal(i.state.status)).length;
    ...
    this.deps.events.emit({ name: "WorkflowCreated", definitionId: def.id, instanceId: inst.id, correlationId: inst.correlationId, at: now });
    return inst;
  }
  ...
  find(id: string): WorkflowInstance | undefined { return this.instances.get(id); }
  list(): readonly WorkflowInstance[] { return [...this.instances.values()]; }
  ...
  schedule(definitionId: string, options: { delayMs?: number; intervalMs?: number; cron?: string; variables?: WorkflowVariables }): string {
    ...
    const s = options.cron
      ...
      : options.intervalMs
        ...
        : this.scheduler.scheduleDelayed(def.id, options.delayMs ?? 0, payload, def.policy.priority);
    this.deps.metrics.timerScheduled();
    this.deps.events.emit({ name: "WorkflowScheduled", definitionId: def.id, at: this.deps.clock.now(), data: { scheduleId: s.id, dueAt: s.dueAt } });
    return s.id;
  }
  ...
  pause(instanceId: string): WorkflowInstance {
    ...
    this.deps.events.emit({ name: "WorkflowPaused", instanceId, definitionId: inst.definitionId, at: now });
    return inst;
  }
  ...
  async resume(instanceId: string): Promise<WorkflowExecution> {
    ...
    inst = appendHistory(inst, { at: now, kind: "resumed" }, this.deps.config.maxHistoryPerInstance);
    this.persist(inst);
    this.deps.events.emit({ name: "WorkflowResumed", instanceId, definitionId: def.id, at: now });
    if (inst.waitingOn) {
      return Object.freeze({
        instanceId, definitionId: def.id, status: inst.state.status,
        durationMs: 0, steps: Object.freeze([]), outputs: inst.state.outputs,
      });
    }
    const { instance, execution } = await this.engine.run(def, inst);
    this.persist(instance);
  ...
  cancel(instanceId: string, reason = "cancelled"): WorkflowInstance {
    ...
    inst = appendHistory(inst, { at: now, kind: "cancelled", data: { reason } }, this.deps.config.maxHistoryPerInstance);
    this.persist(inst);
    this.deps.metrics.instanceCancelled();
    this.deps.events.emit({ name: "WorkflowCancelled", instanceId, definitionId: inst.definitionId, at: now, data: { reason } });
    return inst;
  }
  ...
  archive(instanceId: string): WorkflowInstance {
    ...
    inst = appendHistory(inst, { at: now, kind: "archived" }, this.deps.config.maxHistoryPerInstance);
    this.persist(inst);
    this.deps.events.emit({ name: "WorkflowArchived", instanceId, definitionId: inst.definitionId, at: now });
    return inst;
  }

  /** Delivers an external signal (connector / agent / internal event) to a waiting instance. */
  async signal(instanceId: string, signalName: string, payload: Readonly<Record<string, unknown>> = {}): Promise<WorkflowExecution | undefined> {
    const now = this.deps.clock.now();
    let inst = this.get(instanceId);
    if (!inst.waitingOn || inst.waitingOn.kind !== "signal" || inst.waitingOn.signalName !== signalName) return undefined;
    const def = this.registry.get(inst.definitionId);
    ...
    this.deps.events.emit({ name: "SignalReceived", instanceId, definitionId: def.id, at: now, data: { signalName, stepId } });
    const { instance, execution } = await this.engine.run(def, inst);
    this.persist(instance);
    await this.afterRun(instance);
    return execution;
  ...
  detectDeadWorkflows(): readonly WorkflowInstance[] {
    ...
    for (const inst of this.list()) {
      ...
      const budget = Math.min(def?.policy.executionBudgetMs ?? Infinity, this.deps.config.deadWorkflowAfterMs);
      if (now - (inst.startedAt ?? inst.createdAt) > budget) {
        let d = transitionInstance(inst, "failed", now, "dead_workflow");
        d = appendHistory(d, { at: now, kind: "failed", data: { reason: "dead_workflow" } }, this.deps.config.maxHistoryPerInstance);
        d = Object.freeze({ ...d, error: "dead_workflow" });
        this.persist(d);
        this.deps.metrics.instanceFailed(now - (d.startedAt ?? d.createdAt));
        this.deps.events.emit({ name: "WorkflowFailed", instanceId: d.id, definitionId: d.definitionId, at: now, data: { reason: "dead_workflow" } });
        dead.push(d);
      }
    }
    return dead;
  ...
  // ---------- internals ----------
  private settleWait(inst: WorkflowInstance, stepId: string, output: Readonly<Record<string, unknown>>, now: number, kind: "timer" | "signal"): WorkflowInstance {
    ...
    next = appendHistory(next, { at: now, kind: kind === "timer" ? "timer" : "signal", stepId }, this.deps.config.maxHistoryPerInstance);
    next = appendHistory(next, { at: now, kind: "step-succeeded", stepId, data: { output } }, this.deps.config.maxHistoryPerInstance);
    next = transitionInstance(next, "running", now, `${kind}:${stepId}`);
    this.persist(next);
    return next;
  }

  private persist(inst: WorkflowInstance): void { this.instances.set(inst.id, inst); }
  ...
  private async afterRun(inst: WorkflowInstance): Promise<void> {
    if (inst.waitingOn?.kind === "timer") {
      this.scheduler.timers.at(inst.waitingOn.dueAt ?? this.deps.clock.now(), { instanceId: inst.id }, inst.priority);
      this.deps.metrics.timerScheduled();
    }
  }
}
