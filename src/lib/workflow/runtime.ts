...
export class WorkflowRuntime {
  ...
  register(def: WorkflowDefinition): WorkflowDefinition {
    return this.manager.register(def);
  }
  definitions(): readonly WorkflowDefinition[] {
    return this.manager.registry.list();
  }
  create(definitionId: string, variables: WorkflowVariables = {}): WorkflowInstance {
    return this.manager.create(definitionId, variables);
  }
  start(instanceId: string): Promise<WorkflowExecution> {
    return this.manager.start(instanceId);
  }
  ...
  pause(id: string): WorkflowInstance {
    return this.manager.pause(id);
  }
  resume(id: string): Promise<WorkflowExecution> {
    return this.manager.resume(id);
  }
  cancel(id: string, reason?: string): WorkflowInstance {
    return this.manager.cancel(id, reason);
  }
  archive(id: string): WorkflowInstance {
    return this.manager.archive(id);
  }
  signal(
    id: string,
    name: string,
  ...
  replay(id: string) {
    return this.manager.replay(id);
  }
  snapshot(id: string) {
    return this.manager.snapshot(id);
  }
  recover(id: string): WorkflowInstance {
    return this.manager.recover(id);
  }
  detectDeadWorkflows(): readonly WorkflowInstance[] {
    return this.manager.detectDeadWorkflows();
  }
  statistics(): WorkflowStatistics {
    return this.manager.statistics();
  }
  metricsSnapshot(): WorkflowMetricsSnapshot {
    return this.metrics.snapshot();
  }
  onEvent(l: WorkflowEventListener): () => void {
