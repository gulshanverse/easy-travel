  ...
  return Object.freeze({
    ...
    state: Object.freeze({
      ...
      outputs:
        output === undefined
          ? inst.state.outputs
          : Object.freeze({ ...inst.state.outputs, [stepId]: output }),
    }),
  });
}
...
export class WorkflowStateMachineEngine {
  ...
  private async invokeStep(
    ...
    const input = Object.freeze({
      ...(step.input ?? {}),
      ...inst.variables,
      __outputs: ctx.outputs,
    });
    const call = (): Promise<unknown> => {
      switch (step.kind) {
        case "capability":
          return this.deps.ctor.invokeCapability({
        ...
        case "connector":
          return this.deps.integration.invokeConnector({
        ...
        case "agent":
          return this.deps.agent.reason({
            agentId: step.capabilityId ?? "travel-orchestrator",
    ...
    try {
      return await Promise.race([
        ...
        new Promise<never>((_, rej) => {
          timer = setTimeout(() => rej(new WorkflowTimeoutError(timeoutMs)), timeoutMs);
        }),
      ]);
    } finally {
  ...
  async run(def: WorkflowDefinition, start: WorkflowInstance): Promise<EngineRunResult> {
    ...
    outer: for (const layer of layers) {
      const runnable = layer.filter(
        (s) => inst.state.steps[s.id] === "pending" || inst.state.steps[s.id] === "waiting",
      );
      ...
      for (const step of runnable) {
        ...
        if (depBlocked || guardFalse) {
          ...
          inst = appendHistory(
            inst,
            { at: now(), kind: "step-skipped", stepId: step.id },
      ...
      if (waitStep && inst.state.steps[waitStep.id] !== "waiting") {
        ...
        inst = appendHistory(
          inst,
          { at: now(), kind: "step-waiting", stepId: waitStep.id },
          this.deps.maxHistoryPerInstance,
        );
        inst = Object.freeze({
          ...inst,
      ...
      for (let i = 0; i < pending.length; i += concurrency) {
        const chunk = pending
          ...
          .filter((s) => s.kind !== "timer" && s.kind !== "signal");
        if (!chunk.length) continue;
        const settled = await Promise.all(
          chunk.map((step) => this.runStepWithRetry(def, step, inst)),
        );
        for (const r of settled) {
          ...
          if (r.result.status === "succeeded") {
            ...
            if (this.deps.checkpointEveryStep) {
              ...
              inst = Object.freeze({
                ...inst,
                checkpoints: Object.freeze([...inst.checkpoints, cp]),
              });
              inst = appendHistory(
                inst,
                {
            ...
            events.emit({
              ...
              definitionId: def.id,
              instanceId: inst.id,
              data: { stepId: r.result.id, error: r.result.error },
              at: r.result.endedAt,
            });
            const step = byId.get(r.result.id)!;
            if (step.required !== false)
              failed = { stepId: r.result.id, error: r.result.error ?? "step failed" };
          }
        }
    ...
    if (failed) {
      ...
      inst = appendHistory(
        inst,
        { at: now(), kind: "failed", data: { stepId: failed.stepId, error: failed.error } },
        this.deps.maxHistoryPerInstance,
      );
      ...
      const outstanding = def.steps.some(
        (s) => inst.state.steps[s.id] === "pending" || inst.state.steps[s.id] === "waiting",
      );
      if (!outstanding) {
  ...
  private async runStepWithRetry(
    ...
    step: WorkflowStep,
    inst: WorkflowInstance,
  ): Promise<{ result: WorkflowStepResult }> {
    ...
    while (attempts < Math.max(1, retry.maxAttempts)) {
      ...
      events.emit({
        ...
        definitionId: def.id,
        instanceId: inst.id,
        data: { stepId: step.id, attempt: attempts },
        at: now(),
      });
      try {
  ...
  private async compensate(
    ...
    inst = appendHistory(
      inst,
      { at: now(), kind: "compensation-started" },
      this.deps.maxHistoryPerInstance,
    );
    events.emit({
    ...
    inst = appendHistory(
      inst,
      { at: now(), kind: "compensation-completed" },
      this.deps.maxHistoryPerInstance,
    );
    events.emit({
