...
export function validateWorkflowDefinition(def: WorkflowDefinition): void {
  ...
  if (!SEMVER.test(def.version)) throw new WorkflowValidationError(`Invalid workflow version: ${def.version}`);
  if (def.steps.length === 0) throw new WorkflowValidationError("Workflow requires at least one step");
  ...
  for (const s of def.steps) {
    ...
    if (s.kind === "connector" && (!s.connectorId || !s.capabilityId)) {
      throw new WorkflowValidationError(`Step ${s.id} of kind connector requires connectorId and capabilityId`);
    }
    ...
    if (s.kind === "timer" && (s.delayMs === undefined || s.delayMs < 0)) {
      throw new WorkflowValidationError(`Step ${s.id} of kind timer requires a non-negative delayMs`);
    }
  }
  for (const s of def.steps) {
    for (const d of s.dependsOn) {
      if (!ids.has(d)) throw new WorkflowValidationError(`Step ${s.id} depends on unknown step ${d}`);
    }
  }
  if (def.policy.maxStepConcurrency < 1) throw new WorkflowValidationError("maxStepConcurrency must be >= 1");
...
export function topologicalSort(steps: readonly WorkflowStep[]): readonly WorkflowStep[] {
  ...
  const queue = steps.filter(s => (indegree.get(s.id) ?? 0) === 0).map(s => s.id).sort();
...
export function computeLayers(steps: readonly WorkflowStep[]): readonly (readonly WorkflowStep[])[] {
  const sorted = topologicalSort(steps);
  const depth = new Map<string, number>();
  const byId = new Map(steps.map(s => [s.id, s]));
  for (const s of sorted) {
    const d = s.dependsOn.length
      ? Math.max(...s.dependsOn.map(x => (depth.get(x) ?? 0) + 1))
      : 0;
    depth.set(s.id, d);
  }
