...
describe("MemoryRetriever pipeline", () => {
  it("returns session-scoped context first", async () => {
    ...
    await m.write(d({ class: "conversation", kind: "conversation/turn", scope: "thread", threadId: "t1", payload: { text: "we discussed tokyo" } }));
    await m.write(d({ payload: { text: "likes sushi and ramen" } }));
    const res = await m.retrieve({ ownerId: OWNER, purpose: "companion_turn", text: "tokyo", threadId: "t1" });
  ...
  it("applies per-class caps and item cap", async () => {
    ...
    const res = await m.retrieve({
      ownerId: OWNER, purpose: "companion_turn", text: "pref",
  ...
  it("is deterministic for identical inputs", async () => {
    const m = new MemoryManager();
    for (let i = 0; i < 5; i++) await m.write(d({ payload: { text: `t ${i}` }, kind: `preference/kind-${i}` }));
    const q = { ownerId: OWNER, purpose: "companion_turn" as const, text: "t", now: 1_800_000_000_000 };
  ...
  it("degrades under diversity floor without dropping items", async () => {
    const m = new MemoryManager();
    for (let i = 0; i < 3; i++) await m.write(d({ payload: { text: `p${i}` }, kind: `preference/k-${i}` }));
    const res = await m.retrieve({
      ownerId: OWNER, purpose: "companion_turn", text: "p",
