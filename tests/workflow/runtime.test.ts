...
import {
  ...
  builtinWorkflows,
  WORKFLOW_RUNTIME_ENGINE_CONTRACT,
  WORKFLOW_RUNTIME_CAPABILITY_MANIFEST,
  ExecutionQueue,
...
function rt(opts: Parameters<typeof createWorkflowRuntime>[0] = {}) {
  ...
  return {
    clock,
    telemetry,
    runtime: createWorkflowRuntime({ clock, telemetry, registerBuiltins: false, ...opts }),
  };
}

const linear = () =>
  WorkflowFactory.builder({ id: "wf.linear", name: "Linear", version: "1.0.0" })
...
describe("definitions & validation", () => {
  ...
  it("rejects bad versions, unknown deps and cycles", () => {
    expect(() =>
      WorkflowFactory.definition({
        name: "x",
        version: "1.0",
        steps: [{ id: "a", name: "A", kind: "noop", dependsOn: [] }],
      }),
    ...
    expect(() =>
      WorkflowFactory.definition({
        name: "x",
        version: "1.0.0",
        steps: [{ id: "a", name: "A", kind: "noop", dependsOn: ["z"] }],
      }),
    ).toThrow(WorkflowValidationError);
    expect(() =>
      WorkflowFactory.definition({
...
describe("execution", () => {
  ...
  it("skips dependents of failed steps and fails the instance", async () => {
    ...
    runtime.register(
      WorkflowFactory.builder({ id: "wf.f", name: "F", version: "1.0.0" })
  ...
  it("evaluates conditional transitions", async () => {
    const { runtime } = rt();
    runtime.register(
      WorkflowFactory.builder({ id: "wf.c", name: "C", version: "1.0.0" })
        .step({ id: "a", name: "A", capabilityId: "cap.a", dependsOn: [] })
        .step({
          id: "b",
          name: "B",
          capabilityId: "cap.b",
          dependsOn: ["a"],
          when: (ctx) => ctx.variables.go === true,
        })
        .build(),
    );
  ...
  it("runs parallel branches and joins", async () => {
    ...
    const { runtime } = rt({ ports: { ctor: ctorPort(new Set(), calls) } });
    runtime.register(
      WorkflowFactory.builder({ id: "wf.par", name: "Par", version: "1.0.0" })
        .step({ id: "a", name: "A", capabilityId: "cap.a", dependsOn: [] })
...
describe("retry, timeout & compensation", () => {
  it("retries failing steps up to maxAttempts", async () => {
    ...
    runtime.register(
      WorkflowFactory.builder({ id: "wf.r", name: "R", version: "1.0.0" })
        .step({
          id: "a",
          name: "A",
          capabilityId: "cap.a",
  ...
  it("times out a hanging step", async () => {
    ...
    runtime.register(
      WorkflowFactory.builder({ id: "wf.t", name: "T", version: "1.0.0" })
        .withPolicy({ retry: { maxAttempts: 1, initialDelayMs: 0, multiplier: 1, maxDelayMs: 0 } })
  ...
  it("compensates succeeded steps in reverse order", async () => {
    ...
    runtime.register(
      WorkflowFactory.builder({ id: "wf.comp", name: "Comp", version: "1.0.0" })
        .withPolicy({ retry: { maxAttempts: 1, initialDelayMs: 0, multiplier: 1, maxDelayMs: 0 } })
        .step({
          id: "a",
          name: "A",
          capabilityId: "cap.a",
          dependsOn: [],
          compensation: { capabilityId: "cap.a.undo" },
        })
        .step({ id: "b", name: "B", capabilityId: "cap.b", dependsOn: ["a"] })
        .build(),
    );
  ...
  it("detects dead workflows past their budget", async () => {
    ...
    runtime.register(
      WorkflowFactory.builder({ id: "wf.dead", name: "Dead", version: "1.0.0" })
...
describe("scheduler, timers & signals", () => {
  ...
  it("resumes timer waits", async () => {
    const { runtime, clock } = rt();
    runtime.register(
      WorkflowFactory.builder({ id: "wf.timer", name: "Timer", version: "1.0.0" })
  ...
  it("continues on external signals", async () => {
    const { runtime } = rt();
    runtime.register(
      WorkflowFactory.builder({ id: "wf.sig", name: "Sig", version: "1.0.0" })
...
describe("pause, cancel, checkpoints & recovery", () => {
  it("pauses and resumes a waiting workflow", async () => {
    const { runtime } = rt();
    runtime.register(
      WorkflowFactory.builder({ id: "wf.p", name: "P", version: "1.0.0" })
  ...
  it("cancels and archives", async () => {
    const { runtime } = rt();
    runtime.register(
      WorkflowFactory.builder({ id: "wf.x", name: "X", version: "1.0.0" })
        .step({ id: "s", name: "Await", kind: "signal", signalName: "go", dependsOn: [] })
        .build(),
    );
  ...
  it("two identical runs produce identical history shapes", async () => {
    const shape = async () => {
      ...
      return runtime
        ...
        .history.map((h) => `${h.seq}:${h.kind}:${h.stepId ?? ""}`);
    };
...
describe("automation policies", () => {
  it("enforces max concurrent instances", async () => {
    const { runtime } = rt();
    runtime.register(
      WorkflowFactory.builder({ id: "wf.lim", name: "Lim", version: "1.0.0" })
  ...
  it("enforces rate limits", () => {
    const { runtime } = rt();
    runtime.register(
      WorkflowFactory.builder({ id: "wf.rate", name: "Rate", version: "1.0.0" })
        .withPolicy({ rateLimitPerMinute: 2 })
        .step({ id: "a", name: "A", capabilityId: "cap.a", dependsOn: [] })
        .build(),
    );
    runtime.create("wf.rate");
    runtime.create("wf.rate");
  ...
  it("registers six deterministic travel workflows", async () => {
    ...
    expect(
      builtinWorkflows()
        .map((d) => d.id)
        .sort(),
    ).toEqual([...BUILTIN_WORKFLOW_IDS].sort());
...
describe("concurrency & stress", () => {
  it("runs 200 workflows in parallel", async () => {
    const { runtime } = rt();
    runtime.register(
      WorkflowFactory.builder({ id: "wf.s", name: "S", version: "1.0.0" })
  ...
  it("creates 10,000 concurrent workflow instances", async () => {
    ...
    runtime.register(
      WorkflowFactory.builder({ id: "wf.big", name: "Big", version: "1.0.0" })
        .withPolicy({ maxConcurrentInstances: 100_000, rateLimitPerMinute: 1_000_000 })
        .step({ id: "s", name: "Await", kind: "signal", signalName: "go", dependsOn: [] })
        .build(),
    );
...
describe("architecture fitness", () => {
  ...
  it("publishes a stable engine contract and capability manifest", () => {
    ...
    expect(WORKFLOW_RUNTIME_ENGINE_CONTRACT.dependencies.frozenEngines).toEqual([
      "agent.runtime",
      "ctor.runtime",
      "integration.runtime",
    ]);
  ...
  it("exposes the facade alias", () => {
    expect(WorkflowRuntimeFacade).toBe(
      createWorkflowRuntime({ registerBuiltins: false }).constructor,
    );
  });
